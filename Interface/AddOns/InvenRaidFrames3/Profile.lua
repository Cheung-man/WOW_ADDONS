...
if not RAID_CLASS_COLORS.EVOKER then
	RAID_CLASS_COLORS.EVOKER = { r = 1, g = 1, b = 1 }
end

IRF3.classes = { "WARRIOR", "DRUID", "PALADIN", "DEATHKNIGHT", "PRIEST", "SHAMAN", "ROGUE", "MAGE", "WARLOCK", "HUNTER", "MONK", "DEMONHUNTER", "EVOKER" }
IRF3.roles = { "TANK", "HEALER", "DAMAGER", "NONE" }	-- { "MAINTANK", "MAINASSIST", "TANK", "HEALER", "DAMAGER", "NONE" }
...
local default = {
	...
	partyTag = true, partyTagParty = { 0.90, 0.48, 0.66, 0.8 }, partyTagRaid = { 0, 0, 0, 0.8 },	-- 0.7, 0.28, 0.28, 0.8
	anchor = "TOPLEFT",
	...
	classshown = { WARRIOR = true, DRUID = true, PALADIN = true, DEATHKNIGHT = true, PRIEST = true, SHAMAN = true, ROGUE = true, MAGE = true, WARLOCK = true, HUNTER = true, MONK = true, DEMONHUNTER = true, EVOKER = true },
	roleshown = { true, true, true, true },
	roleorder = IRF3.roles,
	useManager = true, managerPos = 25,
	castingSent = 0,	-- 0:사용 안함 1: 항상 표시 2: 마우스 오버시 표시
	enableFadeIn = false, enableSpellTimerType1 = false,
	units = {
		backgroundColor = { 0.10, 0.10, 0.10, 0.55 },
		orientation = 1,
		displayPowerBar = true, powerBarHeight = 0.10, powerBarPos = 2,
		displayHealPrediction = true, healPredictionAlpha = 0.80, myHealPredictionColor = colorGreen, otherHealPredictionColor = colorYellow, AbsorbPredictionColor = { 0, 0.7, 1 },
		healIcon = true, healIconOther = true, healIconPos = "BOTTOMLEFT", healIconSize = 10,
		displayRaidRoleIcon = true, roleIconPos = "TOPLEFT", roleIconSize = 12, centerStatusIcon = true, roleIcontype = 0, hideDamagerIcon = false,
		...
		dispelSound = "None", dispelSoundDelay = 5, useHarm = true, onlyDispel = false, showDispelTimer = false,
		outline = {
		...
		RaidCheck=false, flaskFlags=70, foodFlags=18, runeFlags=18, showRunes=false, annFood=true, annFlask=true, annRune=true, RaidCheckAnn=false,
	...
	colors = {
		...
		EVOKER = { RAID_CLASS_COLORS.EVOKER.r, RAID_CLASS_COLORS.EVOKER.g, RAID_CLASS_COLORS.EVOKER.b },
		--- added 7.0.3 legion class... DEMON HUNTER
...
function IRF3:InitDB()
	...
	if not InvenRaidFrames3DB then
		InvenRaidFrames3DB = {
			profileKeys = {},
			profiles = {},
			minimapButton = { show = true, radius = 80, angle =  15, dragable = true, rounding = 10 },
		}
	end
...
function IRF3:UpdateGroupFilter()
	...
	elseif self.db.groupby == "CLASS" then
		for i = 1, 8 do
			self.headers[i].partyTag:Hide()
			setPet(self.headers[i])
		end
	...
	elseif self.db.groupby == "ROLE" then	-- ROLE
		for i = 1, 8 do
			self.headers[i].partyTag:Hide()
			if self.db.groupshown[i] then
				groupfilter = (groupfilter and (groupfilter..",") or "")..i
		...
		for i, role in ipairs(self.db.roleorder) do
			if self.db.roleshown[i] then
				rolefilter = (rolefilter and (rolefilter..",") or "")..role
			end
		end
	end
	...
	self:SetAttribute("rolefilter", rolefilter or "0")

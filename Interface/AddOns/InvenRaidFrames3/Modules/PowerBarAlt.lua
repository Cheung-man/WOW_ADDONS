...
function InvenRaidFrames3Member_UpdatePowerBarAlt(self)
	if self.powerBarAlt then
		if IRF3.db.units.usePowerBarAlt and GetUnitPowerBarInfo then	-- classic

			local barInfo = GetUnitPowerBarInfo(self.displayedUnit);
			if barInfo and barInfo.barType then
				barType = barInfo.barType;
				minPower = barInfo.minPower or 0;
			end

			if barType then
				minPower = minPower or 0
				self.powerBarAlt.r, self.powerBarAlt.g, self.powerBarAlt.b = select(2, GetUnitPowerBarTextureInfo(self.displayedUnit, 2))

...
function IRF3:SelectClickCastingDB()
	...
	if GetSpecialization then
		...
		IRF3.specId, specId = 1, 1
	end
-- 9.0.1 패치 특성 번호가 5번으로 옴 	Returns a value of 5 as of 9.0.1 for newly created characters.
	--IRF3.talent, talent = GetSpecialization() or 1, IRF3.talent
	talent = GetSpecialization and GetSpecialization() or 1
	if talent == 5 then
		talent = 1
	end
	IRF3.talent, talent = talent, IRF3.talent

-- 9.0.1 specId 쓰는 부분 주석
	if IRF3.talent ~= talent then	--if IRF3.specId ~= specId or IRF3.talent ~= talent then

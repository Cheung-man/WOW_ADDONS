...
function addon:SetupDefaultFilters()
	...
	local CONSUMMABLE = L['Consumable'] --GetItemClassInfo(LE_ITEM_CLASS_CONSUMABLE)
	local GEM = L['Gem'] --GetItemClassInfo(LE_ITEM_CLASS_GEM)
	local GLYPH = L['Glyph'] --GetItemClassInfo(LE_ITEM_CLASS_GLYPH)
	local JUNK = L['Junk'] --GetItemSubClassInfo(LE_ITEM_CLASS_MISCELLANEOUS, 0)
	local MISCELLANEOUS = L['Miscellaneous'] --GetItemClassInfo(LE_ITEM_CLASS_MISCELLANEOUS)
	local QUEST = L['Quest Items'] --GetItemClassInfo(LE_ITEM_CLASS_QUESTITEM)
	local RECIPE = L['Recipe'] --GetItemClassInfo(LE_ITEM_CLASS_RECIPE)
	local TRADE_GOODS = L['Trade Goods'] --GetItemClassInfo(LE_ITEM_CLASS_TRADEGOODS)
	local WEAPON = L['Weapon'] --GetItemClassInfo(LE_ITEM_CLASS_WEAPON)
	local ARMOR = L['Armor'] --GetItemClassInfo(LE_ITEM_CLASS_ARMOR)

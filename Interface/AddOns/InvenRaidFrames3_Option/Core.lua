...
local menuList = {
	["기본"] = {
		...
		{ name = "[주문 타이머]", desc = "버프 및 디버프를 아이콘과 남은 시간으로 표시합니다. 버프/디버프 이름이나 주문 ID를 입력후 꼭 엔터키를 눌러야 변경 사항이 적용됩니다.", func = menuOnClick, create = "SpellTimer" },
		{ name = "프로필 관리", desc = "캐릭터 별로 자신이 설정한 프로필로 간단히 변경 가능하게 해줍니다.", func = menuOnClick, create = "Profile", disableScroll = true },
		{ name = "실험실", desc = "새로운 기능을 테스트 합니다.", func = menuOnClick, create = "Advanced", disableScroll = true },
	},
	["외형"] = {
	...
	["기능"] = {
		...
		{ name = "도핑 체크", desc = "전투 준비 시 영약 및 음식, 증강의 룬 도핑이 되었는지 공대 창으로 알려주는 기능입니다.", func = menuOnClick, create = "RaidCheck" },
		{ name = "파티장 아이콘", desc = "파티장 또는 공격대장을 알려주는 아이콘을 표시합니다.", func = menuOnClick, create = "LeaderIcon" },
	},
}
...
for i, name in ipairs(tabList) do
	...
	Option.mainTab[i]:ClearAllPoints()
	Option.mainTab[i]:SetPoint("LEFT", Option.mainTab[i - 1], "RIGHT", -16, 0)
	Option.mainTab[i]:SetID(i)
	Option.mainTab[i]:SetText(name)

	Option.mainTab[i].Text = _G[Option.mainTab[i]:GetName() .. "Text"]
	Option.mainTab[i].Left = _G[Option.mainTab[i]:GetName() .. "Left"]
	Option.mainTab[i].Right = _G[Option.mainTab[i]:GetName() .. "Right"]
	Option.mainTab[i].Middle = _G[Option.mainTab[i]:GetName() .. "Middle"]
	Option.mainTab[i].LeftActive = Option.mainTab[i].LeftDisabled or _G[Option.mainTab[i]:GetName().."LeftDisabled"];
	Option.mainTab[i].MiddleActive = Option.mainTab[i].MiddleDisabled or _G[Option.mainTab[i]:GetName().."MiddleDisabled"];
	Option.mainTab[i].RightActive = Option.mainTab[i].RightDisabled or _G[Option.mainTab[i]:GetName().."RightDisabled"];
	Option.mainTab[i].Text:SetText(name)
	Option.mainTab[i].Text:SetPoint("LEFT", 20, -2)
	
	Option.mainTab[i]:HookScript("OnShow", function()
		_G[Option.mainTab[i]:GetName() .. "Middle"]:SetWidth(Option.mainTab[i].Text:GetWidth())
		_G[Option.mainTab[i]:GetName() .. "MiddleDisabled"]:SetWidth(Option.mainTab[i].Text:GetWidth())
		_G[Option.mainTab[i]:GetName() .. "HighlightTexture"]:SetPoint("RIGHT", 10, -4)
	end)
	Option:Show()

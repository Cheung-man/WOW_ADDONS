...
function Option:CreateClickCastingMenu(menu, parent)
	...
	do
		...
		if GetNumSpecializations then	-- classic
	...
	local function checkSpell(spell, index, type)
		if spell and spell ~= "" then
			if not IsPassiveSpell(index, type) then	--IsUsableSpell(index, type) and
				return true
			elseif not IsPassiveSpell(spell) then --and IsUsableSpell(spell) then
				return true
			elseif overrideSpells[spell] then
				if IsUsableSpell(overrideSpells[spell]) and not IsPassiveSpell(overrideSpells[spell]) then
					return true
				end
	...
	local function hasSpellTab(tab)
		name, _, start, last, isGuild, offSpecID = GetSpellTabInfo(tab)
		local isOffSpec = (offSpecID ~= 0);
		if name and not isOffSpec then
			local cnt = 0
		...
		if level then
			...
			elseif level == 2 then
				if type(UIDROPDOWNMENU_MENU_VALUE) == "number" then
					...
					for tabIndex = 1, GetNumSpellTabs() do
						name, _, start, last, isGuild, offSpecID = GetSpellTabInfo(tabIndex)
						local isOffSpec = (offSpecID ~= 0);
						if name and not isOffSpec then
							for i = start + 1, start + last do
	...
	local function getKeyValue(modifilter)
		...
		if not IRF3.ccdb then
		   IRF3.ccdb = InvenRaidFrames3CharDB.clickCasting[IRF3.talent]
		 end
		--if not IRF3.ccdb[modifilter] then
		--   IRF3.ccdb[modifilter] = NONE
		-- end

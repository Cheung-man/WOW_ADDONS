...
function Option:CreateFrameMenu(menu, parent)
	...
	menu.texture = LBO:CreateWidget("Media", parent, "바 텍스쳐", "바 텍스쳐를 설정합니다.", nil, nil, true,
	...
	menu.highlightAlpha = LBO:CreateWidget("Slider", parent, "하이라이트 투명도", "프레임에 마우스를 올렸을때 나오는 하이라이트 텍스쳐의 투명도를 설정합니다. 0으로 설정하면 보이지 않습니다.", nil, nil, true,
		function()
	...
	menu.color = LBO:CreateWidget("ColorPicker", parent, "배경 색상", "각 플레이어 프레임의 뒷 배경 색상 및 투명도를 설정합니다.", nil, nil, true,
		function()
...
function Option:CreateHealthBarMenu(menu, parent)
	...
	menu.orientation = LBO:CreateWidget("DropDown", parent, "체력바 방향", "체력바의 진행 방향을 설정합니다.", nil, nil, true,
		function()
	...
	menu.classColor = LBO:CreateWidget("CheckBox", parent, "직업별 체력바 색상", "직업별 색상에 따라 체력바 색상을 변경합니다.", nil, nil, true,
		function()
	...
	menu.reset = LBO:CreateWidget("Button", parent, "색상 초기화", "설정한 색상을 초기값으로 되돌립니다.", nil, nil, true,
		function()
	...
	local colorList = { "help", "harm", "pet", "vehicle", "offline", "WARRIOR", "ROGUE", "PRIEST", "MAGE", "WARLOCK", "HUNTER", "DRUID", "SHAMAN", "PALADIN", "DEATHKNIGHT", "MONK", "DEMONHUNTER", "EVOKER" }
	local colorLocale = { "우호적 대상", "적대적 대상", "소환수", "탈것 탑승 시", "오프라인일 때", "전사", "도적", "사제", "마법사", "흑마법사", "사냥꾼", "드루이드", "주술사", "성기사", "죽음의 기사", "수도사", "악마사냥꾼", "기원사" }
...
function Option:CreateManaBarMenu(menu, parent)
	...
	menu.pos = LBO:CreateWidget("DropDown", parent, "마나바 위치", "마나바의 위치를 설정합니다.", nil, nil, true,
		function()
	...
	menu.height = LBO:CreateWidget("Slider", parent, "크기 비율", "마나바의 크기 비율을 설정합니다. 0%로 설정하면 마나바가 숨겨지며 100%로 설정하면 체력바가 숨겨집니다.", nil, nil, true,
		function()
...
function Option:CreateNameMenu(menu, parent)
	...
	menu.offlineName = LBO:CreateWidget("CheckBox", parent, "오프라인 플레이어 직업별 이름 색상 사용", "오프라인인 플레이어의 이름 색상을 직업별 색상으로 표시합니다.", nil, nil, true,
		function()
		...
		function(r, g, b, a)
			...
			for i = 1, 8 do
				if i ~= IRF3.playerGroup then
					IRF3.headers[i].partyTag.tex:SetColorTexture(r, g, b, a);
				end
			end
			Option:UpdatePreview()
		end
...
function Option:CreateBorderMenu(menu, parent)
	menu.use = LBO:CreateWidget("CheckBox", parent, "배경 테두리 보기", "공격대 프레임 전체를 둘러싸는 테두리를 보입니다.", nil, nil, true,
	...
	menu.reset = LBO:CreateWidget("Button", parent, "색상 초기화", "설정한 색상을 초기값으로 되돌립니다.", nil, disable, true,
		function()
	...
	menu.backdrop = LBO:CreateWidget("ColorPicker", parent, "배경 테두리 내부 색상", "공격대 프레임 전체를 둘러싸는 테두리의 내부 색상 및 투명도를 조절합니다.", nil, disable, true,
		function()
	...
	menu.border = LBO:CreateWidget("ColorPicker", parent, "배경 테두리 외각선 색상", "공격대 프레임 전체를 둘러싸는 테두리의 외각선 색상 및 투명도를 조절합니다.", nil, disable, true,
		function()
	...
	menu.borderEdgeValue = LBO:CreateWidget("DropDown", parent, "테두리 스킨 변경", "테두리 스킨을 변경합니다.", nil, nil, true,
		function()
...
function Option:CreateDebuffColorMenu(menu, parent)
	...
	for i, color in ipairs(colorList) do
		menu["color"..i] = LBO:CreateWidget("ColorPicker", parent, colorLocale[i], colorLocale[i].." 디버프의 색상을 변경합니다.", nil, nil, true, getColor, setColor, color)
		if i == 1 then
			menu["color"..i]:SetPoint("TOP", menu.reset, "BOTTOM", 0, 15)
		elseif i == 2 then
			menu["color"..i]:SetPoint("TOP", menu.color1, 0, 0)

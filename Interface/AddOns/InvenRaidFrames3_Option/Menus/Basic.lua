...
function Option:CreateUseMenu(menu, parent)
	...
	local useList = { "항상", "파티 및 공격대", "공격대", "파티" }
	...
	menu.lock = LBO:CreateWidget("CheckBox", parent, "창 고정", "공격대 프레임을 잠가 이동시키지 못하게 고정합니다. ", nil, nil, true,
	...
	menu.lockAlt = LBO:CreateWidget("CheckBox", parent, "Alt이동", "잠금 상태일 때, 공격대 프레임을 움직이려면 Alt 키를 누른 상태에서 움직이세요. ", nil, nil, true,
	...
	menu.manager = LBO:CreateWidget("CheckBox", parent, "공격대 관리자 사용하기", "화면 우측 상단에 있는 공격대 관리자(공격대원)창의 사용 여부를 설정합니다.", nil, nil, true,
	...
	menu.castingSent = LBO:CreateWidget("DropDown", parent, "시전 표시기", "주문이 시전 되었을 때 프레임에 효과를 줍니다.", nil, nil, true,
...
function Option:CreateGroupMenu(menu, parent)
	...
	local groupbyList = { "파티별", "직업별", "역할별" }
	menu.groupby = LBO:CreateWidget("DropDown", parent, "정렬 방식", "공격대 그룹을 파티별 혹은 직업별로 정렬합니다.", nil, nil, true,
		function() return IRF3.db.groupby == "GROUP" and 1 or (IRF3.db.groupby == "CLASS" and 2 or 3), groupbyList end,
		function(v)
			IRF3.db.groupby = v == 1 and "GROUP" or (v == 2 and "CLASS" or "ROLE")
			menu.group:Update()
			menu.class:Update()
			menu.role:Update()
	...
	local function setGroupPos(self)
		if SetUserPlaced then self:SetUserPlaced(nil) end
		self:ClearAllPoints()
		if SetUserPlaced then self:SetUserPlaced(nil) end
	...
	for i = 1, 8 do
		Option.group[i] = CreateFrame("Button", Option:GetName().."GroupButton"..i, menu.group.visible, "IRF_CRFManagerFilterGroupButtonTemplate")
	...
	for i, class in ipairs(IRF3.classes) do
		Option.class[class] = CreateFrame("Button", Option:GetName().."ClassButton"..i, menu.class.visible, "IRF_CRFManagerFilterGroupButtonTemplate")
		...
		Option.class[class].tex = Option.class[class]:CreateTexture(nil, "OVERLAY")
...
-- ROLE ROLE ROLE ROLE ROLE ROLE ROLE ROLE ROLE ROLE ROLE ROLE ROLE ROLE ROLE ROLE ROLE ROLE ROLE ROLE

	menu.role = LBO:CreateWidget("ShowHide", parent, function() return IRF3.db.groupby ~= "ROLE" end)
	menu.role:SetAllPoints(menu.group)
	...
	local function setrolePos(self, index)
		index = index or lookupTable(IRF3.db.roleorder, self.index)
		if SetUserPlaced then self:SetUserPlaced(nil) end
		self:ClearAllPoints()
		if SetUserPlaced then self:SetUserPlaced(nil) end
		self:SetPoint("CENTER", Option.roleAnchor[index], 0, 0)
		if SetUserPlaced then self:SetUserPlaced(nil) end
		self:SetFrameLevel(menu.role:GetFrameLevel() + 1)
	end
	local function roleOnHide(self)
		if Option.role.drag == self then
			Option.role.drag = nil
			if SetUserPlaced then self:SetUserPlaced(nil) end
			self:StopMovingOrSizing()
			if SetUserPlaced then self:SetUserPlaced(nil) end
			self:SetFrameLevel(menu.role:GetFrameLevel() + 1)
	...
	local function roleOnDragStop(self)
		if roleOnHide(self) then
			...
			for i, role in ipairs(IRF3.db.roleorder) do
				if i ~= 4 and self.index ~= role and Option.role[role]:IsMouseOver() then
					local index = lookupTable(IRF3.db.roleorder, self.index)
					IRF3.db.roleorder[index], IRF3.db.roleorder[i] = IRF3.db.roleorder[i], IRF3.db.roleorder[index]
					for _, role in ipairs(IRF3.db.roleorder) do
						setrolePos(Option.role[role])
					end
					return IRF3:UpdateGroupFilter()
				end
			end
		end
		setrolePos(Option.role[self.index])
	end
	local function roleOnDragStart(self)
		Option.role.drag = self
		self:SetFrameLevel(menu.role:GetFrameLevel() + 2)
		if SetUserPlaced then self:SetUserPlaced(nil) end
		self:StartMoving()
		if SetUserPlaced then self:SetUserPlaced(nil) end
	end
	local function roleOnClick(self)
		IRF3.db.roleshown[lookupTable(IRF3.db.roleorder, self.index)] = not IRF3.db.roleshown[lookupTable(IRF3.db.roleorder, self.index)]
		if IRF3.db.roleshown[lookupTable(IRF3.db.roleorder, self.index)] then
	...
	Option.role.Update = function()
		if Option.role.drag then
			roleOnHide(Option.role.drag)
		end
		if menu.role.visible:IsVisible() then
			if InCombatLockdown() or UnitAffectingCombat("player") then
				for i, role in ipairs(IRF3.db.roleorder) do
					setrolePos(Option.role[role], i)
					Option.rolerole[role]:Disable()
					Option.role[role]:SetAlpha(0.5)
					if IRF3.db.roleshown[role] then
						Option.role[role].selectedHighlight:Show()
					else
						Option.role[role].selectedHighlight:Hide()
					end
				end
			else
				for i, role in ipairs(IRF3.db.roleorder) do
					setrolePos(Option.role[role], i)
					Option.role[role]:Enable()
					Option.role[role]:SetAlpha(1)
					if IRF3.db.roleshown[role] then
						Option.role[role].selectedHighlight:Show()
					else
						Option.role[role].selectedHighlight:Hide()
	...
	for i, role in ipairs(IRF3.roles) do
		Option.role[role] = CreateFrame("Button", Option:GetName().."roleButton"..i, menu.role.visible, "CRFManagerFilterGroupButtonTemplate")
		Option.role[role]:GetHighlightTexture():SetAlpha(0.25)
		Option.role[role].index = role
		Option.role[role]:SetFrameLevel(menu.role:GetFrameLevel() + 1)
		Option.role[role]:SetSize(36, 36)
		Option.role[role]:SetMovable(true)
		Option.role[role]:RegisterForDrag("LeftButton")
		Option.role[role]:SetClampedToScreen(true)
		Option.role[role]:SetScript("OnDragStart", roleOnDragStart)
		Option.role[role]:SetScript("OnDragStop", roleOnDragStop)
		Option.role[role]:SetScript("OnHide", roleOnHide)
		Option.role[role]:SetScript("OnClick", roleOnClick)
		Option.role[role].tex = Option.role[role]:CreateTexture(nil, "OVERLAY")
		Option.role[role].tex:SetPoint("CENTER", 0, 0)
		Option.role[role].tex:SetSize(32, 32)
		Option.role[role].tex:SetTexture("Interface\\AddOns\\InvenRaidFrames3\\Texture\\RoleIcon_MiirGui")
		if lookupTable(IRF3.roles, role) == 1 then
			Option.role[role].tex:SetTexCoord(GetTexCoordsForRoleSmallCircle("TANK"))
		elseif lookupTable(IRF3.roles, role) == 2 then
			Option.role[role].tex:SetTexCoord(GetTexCoordsForRoleSmallCircle("HEALER"))
		else
			Option.role[role].tex:SetTexCoord(GetTexCoordsForRoleSmallCircle("DAMAGER"))
		end
		Option.role[role].tex:SetAlpha(0.75)
		Option.roleAnchor[i] = menu.role.visible:CreateTexture()
		Option.roleAnchor[i]:SetSize(36, 36)
	end

	Option.roleAnchor[1]:SetPoint("TOPRIGHT", Option.roleAnchor[2], "TOPLEFT", -2, 0)
	Option.roleAnchor[2]:SetPoint("TOPRIGHT", Option.roleAnchor[3], "TOPLEFT", -2, 0)
	Option.roleAnchor[3]:SetPoint("TOPRIGHT", menu.group.text, "BOTTOM", -1, -20)

	menu.role.visible:SetScript("OnShow", Option.role.Update)
	menu.role.visible:SetScript("OnEvent", Option.role.Update)
	menu.role.visible:RegisterEvent("PLAYER_ENTERING_WORLD")
	menu.role.visible:RegisterEvent("PLAYER_REGEN_DISABLED")
	menu.role.visible:RegisterEvent("PLAYER_REGEN_ENABLED")
end
